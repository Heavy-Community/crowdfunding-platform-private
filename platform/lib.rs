...
mod platform {
    ...
    impl Project {
        ...
        fn update(&mut self, current_time: Timestamp) {
            ...
            else if self.status == ProjectStatus::Ongoing && self.invested_funds >= self.funding_goal {
    ...
    pub struct Platform {
        ...
        /// The _address_ of the Token contract that will be
    ...
    impl Platform {
        ...
        /// **IMPORTANT**: Beforehand investor should `approve` the amount he
        /// wants to invest in the Platform
        #[ink(message)]
        pub fn invest_funds(
            &mut self,
            amount: Balance,
            ...
            self.ongoing_projects.get(project_id).unwrap().invest_funds(amount)?;
            self.ongoing_projects.get(project_id).unwrap().update(self.env().block_timestamp());
        ...
        pub fn withdraw_funds(&self, project_id: u128) -> Result<()> {
            ...
            let reached_funding = self.ongoing_projects.get(project_id).unwrap().invested_funds;
        ...
        /// TODO: Discuss with Niko whether this shall be implemented rn
        ...
        /// Checks whether `project` has been finished succesfully
